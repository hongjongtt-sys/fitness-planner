/* ================================================
   홈트 플래너 v4 — 2주마다 종목 교체 + 강도 증가
   시작일: 2026-05-30 / 중급 기준
   키: 182cm / 112kg → 90kg / 4~5개월
   장비: 스텝퍼, 스텝박스, 월볼·메디신볼(소음없는것만), 맨몸
   특이사항: 심폐 약함, 알배김 강함
   ================================================ */
...
// ===================================================
// 루틴 설계 원칙
// 월: 하체 근력  화: 유산소  수: 상체·코어
// 목: 유산소·서킷  금: 전신근력  토: 리커버리  일: 휴식
// 2주마다 종목 교체 — 같은 부위, 다른 동작
// 소음 없는 동작만 사용
// ===================================================
...
const PHASE_PLANS = {
  ...
  1: {
    ...
    1: {
      type:'strength', label:'하체 A — 기본 근력', totalMin:60,
      note:'기본 패턴 완성 주간. 자세에 집중하세요. 세트 간 45초 휴식.',
      exercises:[
    ...
    2: {
      type:'cardio', label:'유산소 A — 스테디 스테이트', totalMin:60,
      note:'심폐 적응 주간. 강도보다 꾸준한 유지가 목표!',
      exercises:[
    ...
    3: {
      type:'strength', label:'상체·코어 A — 밀기 중심', totalMin:60,
      note:'알배김 강하면 다음날 스트레칭 10분 추가.',
      exercises:[
    ...
    4: {
      type:'cardio', label:'서킷 A — 3라운드 입문', totalMin:60,
      note:'서킷 첫 주. 동작 사이 멈추지 않는 것에 집중!',
      exercises:[
    ...
    5: {
      type:'strength', label:'전신 A — 하체+상체 복합', totalMin:60,
      note:'금요일은 전신 마무리. 완료 후 단백질 섭취!',
      exercises:[
  ...
  2: {
    0: { type:'rest', label:'완전 휴식', exercises:[] },
    1: {
      type:'strength', label:'하체 B — 단방향 + 볼 저항', totalMin:60,
      note:'종목 교체! 이번 주부터 휴식 30초로 단축.',
      exercises:[
    ...
    2: {
      type:'cardio', label:'유산소 B — 인터벌 강화', totalMin:60,
      note:'강한 구간 비중 늘리기 시작!',
      exercises:[
        { name:'준비운동 — 제자리 걷기',             detail:'5분',                             tag:'warm',     min:5 },
    ...
    3: {
      type:'strength', label:'상체·코어 B — 각도 변화', totalMin:60,
      note:'같은 부위, 다른 각도로 자극. 세트 수 증가!',
      exercises:[
        { name:'준비운동 — 암 서클 + 토르소 트위스트', detail:'5분',                          tag:'warm',     min:5 },
    ...
    4: {
      type:'cardio', label:'서킷 B — 4라운드 + 볼 동작', totalMin:60,
      note:'서킷 라운드 4회로 증가! 볼 동작 추가.',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분',                             tag:'warm',     min:5 },
    ...
    5: {
      type:'strength', label:'전신 B — 하체+코어 복합', totalMin:60,
  ...
  3: {
    0: { type:'rest', label:'완전 휴식', exercises:[] },
    1: {
      type:'strength', label:'하체 C — 슈퍼세트 도입', totalMin:60,
      note:'슈퍼세트: 두 동작을 휴식 없이 연달아! 라운드 사이만 30초.',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'6분 (분당 80 스텝)',              tag:'warm',     min:6 },
    ...
    2: {
      type:'cardio', label:'유산소 C — 인터벌 피크', totalMin:60,
      note:'강한 구간이 약한 구간보다 길어요. 버텨보세요!',
      exercises:[
        { name:'준비운동 — 제자리 걷기',             detail:'5분',                             tag:'warm',     min:5 },
    ...
    3: {
      type:'strength', label:'상체·코어 C — 슈퍼세트', totalMin:60,
      note:'가슴→어깨 연속 자극. 펌핑 극대화!',
      exercises:[
        { name:'준비운동 — 암 서클 + 토르소 트위스트', detail:'5분',                          tag:'warm',     min:5 },
    ...
    4: {
      type:'cardio', label:'서킷 C — 5종목 복합', totalMin:60,
      note:'서킷 종목 5개로 늘었어요. 전신이 다 타는 느낌!',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분',                             tag:'warm',     min:5 },
    ...
    5: {
      type:'strength', label:'전신 C — 슈퍼세트 복합', totalMin:60,
      note:'버피 이제 점프 버전으로! 힘들면 2세트만 점프.',
      exercises:[
    ...
    6: {
      type:'cardio', label:'액티브 리커버리', totalMin:45,
      note:'5~6주차 피로 피크. 충분히 쉬어야 다음 주 HIIT를 버텨요!',
      exercises:[
        { name:'스텝퍼 저강도 유산소',               detail:'30분 (심박 115~125)',             tag:'cardio',   min:30 },
  ...
  4: {
    0: { type:'rest', label:'완전 휴식', exercises:[] },
    1: {
      type:'strength', label:'하체 D — 점프 + 볼 저항', totalMin:60,
    ...
    2: {
      type:'cardio', label:'유산소 D — HIIT 전환', totalMin:60,
      note:'스텝퍼 HIIT 핵심 — 고강도 구간에서 심박 155 이상 목표!',
      exercises:[
        { name:'준비운동 — 제자리 걷기',             detail:'5분',                             tag:'warm',     min:5 },
    ...
    3: {
      type:'strength', label:'상체·코어 D — 고반복 슈퍼세트', totalMin:60,
    ...
    4: {
      type:'cardio', label:'서킷 D — 6종목 HIIT', totalMin:60,
      note:'6종목 서킷! 한 라운드 끝날 때마다 30초만 쉬어요.',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분',                             tag:'warm',     min:5 },
    ...
    5: {
      type:'strength', label:'전신 D — 복합 고강도', totalMin:60,
      note:'스텝박스 무게 8kg 도전!',
      exercises:[
  ...
  5: {
    0: { type:'rest', label:'완전 휴식', exercises:[] },
    1: {
      type:'strength', label:'하체 E — 피크 복합', totalMin:65,
      note:'피크 강도 돌입! 충분한 수면이 필수.',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분 (분당 90 스텝)',              tag:'warm',     min:5 },
    ...
    2: {
      type:'cardio', label:'유산소 E — 피크 HIIT', totalMin:60,
      note:'심박 160 이상 구간 만들어보세요!',
      exercises:[
        { name:'준비운동 — 제자리 걷기',             detail:'5분',                             tag:'warm',     min:5 },
    ...
    3: {
      type:'strength', label:'상체·코어 E — 피크 슈퍼세트', totalMin:60,
      note:'세트 사이 멈추지 않는 것이 목표!',
      exercises:[
        { name:'준비운동 — 암 서클 + 토르소 트위스트', detail:'5분',                          tag:'warm',     min:5 },
    ...
    4: {
      type:'cardio', label:'서킷 E — 피크 전신', totalMin:65,
      note:'완주하면 그 주 완전 성공!',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분',                             tag:'warm',     min:5 },
    ...
    5: {
      type:'strength', label:'전신 E — 피크 근력', totalMin:65,
      note:'스텝박스 8~10kg 도전!',
      exercises:[
    ...
    6: {
      ...
      note:'피크 2주차 후 회복. 몸이 많이 지쳐있을 거예요.',
      exercises:[
  ...
  6: {
    0: { type:'rest', label:'완전 휴식', exercises:[] },
    1: {
      type:'strength', label:'하체 F — 심화 유지', totalMin:65,
      note:'이제 자신의 페이스가 잡혀야 해요. 무게는 본인이 조절!',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분 (분당 95 스텝)',              tag:'warm',     min:5 },
    ...
    2: {
      type:'cardio', label:'유산소 F — 심화 유지', totalMin:60,
      note:'지구력 완성 단계. 숨 차면서도 버티는 구간이 길어져야!',
      exercises:[
        { name:'준비운동 — 제자리 걷기',             detail:'5분',                             tag:'warm',     min:5 },
        { name:'스텝박스 스텝업 인터벌',             detail:'50초 강 / 10초 약 × 15세트',      tag:'cardio',   min:25 },
        { name:'스텝퍼 HIIT',                        detail:'1분 고강도 / 40초 저강도 × 12회', tag:'cardio',  min:23 },
    ...
    3: {
      type:'strength', label:'상체·코어 F — 심화 유지', totalMin:60,
      note:'반복 수보다 자세 집중. 질 좋은 세트가 핵심!',
      exercises:[
        { name:'준비운동 — 암 서클 + 토르소 트위스트', detail:'5분',                          tag:'warm',     min:5 },
    ...
    4: {
      type:'cardio', label:'서킷 F — 6라운드 심화', totalMin:65,
      note:'서킷 완주 능력이 매주 느껴지게 올라야 해요.',
      exercises:[
        { name:'준비운동 — 스텝퍼 워밍업',           detail:'5분',                             tag:'warm',     min:5 },
    ...
    5: {
      type:'strength', label:'전신 F — 심화 근력', totalMin:65,
      note:'목표 90kg 가까워질수록 강도 유지가 더 중요해요!',
      exercises:[
...
// ===== 운동 상세 정보 DB =====
const EXERCISE_INFO = {
  '준비운동 — 스텝퍼 워밍업': { target:['심폐계','하체 혈류'], how:['스텝퍼에 올라서서 양발이 페달에 완전히 닿도록 서세요.','등을 곧게 펴고 손잡이는 가볍게만 잡아요.','천천히 분당 65~70 스텝으로 시작합니다.','발 전체로 고르게 압력을 가하세요.'], tip:'손잡이를 잡지 않으면 코어 활성화와 칼로리 소모가 올라요.', caution:'무릎이 안쪽으로 꺾이지 않도록 주의하세요.', youtube:'스텝퍼 올바른 자세' },
  '준비운동 — 제자리 걷기': { target:['심폐계','전신 혈류'], how:['제자리에서 팔을 앞뒤로 흔들며 걷습니다.','무릎을 높이 들수록 강도가 올라가요.','5분간 숨이 차지 않는 속도로 진행.'], tip:'체온을 서서히 올리는 게 목적이에요.', caution:'없음 — 누구나 안전하게 할 수 있어요.', youtube:'제자리 걷기 워밍업' },
  '준비운동 — 암 서클 + 토르소 트위스트': { target:['어깨','등 상부','코어'], how:['암 서클: 팔을 옆으로 뻗어 앞으로 10회, 뒤로 10회 크게 돌려요.','토르소 트위스트: 발 어깨 너비로 서서 팔을 T자로 뻗고 상체만 좌우로 비틀어요.','각 방향 10회씩 2세트.'], tip:'허리가 아닌 흉추(등 위쪽)를 비트는 느낌으로 해야 효과적이에요.', caution:'허리 통증이 있으면 회전 범위를 줄여주세요.', youtube:'상체 워밍업 루틴' },
  '스쿼트': { target:['대퇴사두근 (앞 허벅지)','둔근 (엉덩이)','햄스트링 (뒤 허벅지)','코어'], how:['발을 어깨 너비보다 살짝 넓게, 발끝은 15~30° 바깥으로.','가슴 피고 엉덩이를 뒤로 내밀며 내려가요.','무릎이 발끝 방향과 일치하도록 유지.','허벅지가 바닥과 평행해질 때까지 내려갔다가 발뒤꿈치로 밀어 올라와요.'], tip:'올라올 때 엉덩이를 꽉 쥐는 느낌!', caution:'무릎 통증 시 깊이를 줄이거나 발끝을 더 바깥으로.', youtube:'스쿼트 초보 자세 교정' },
  '점프 스쿼트': { target:['대퇴사두근','둔근','햄스트링','심폐 지구력'], how:['스쿼트 자세로 내려갔다가 올라오는 힘으로 점프.','착지는 발앞꿈치 먼저 → 발 전체로 부드럽게!','쿵 소리 나지 않게 착지 후 바로 다음 스쿼트.'], tip:'관절 충격을 줄이려면 착지 시 무릎을 약간 구부려 흡수하세요.', caution:'무릎 통증 시 즉시 일반 스쿼트로 대체.', youtube:'점프 스쿼트 자세 홈트' },
  '런지 (좌우 교대)': { target:['대퇴사두근','둔근','햄스트링','균형감각'], how:['한 발을 앞으로 크게 내딛어요 (보폭 70~80cm).','앞 무릎 90°, 뒷 무릎은 바닥 직전에 멈춰요.','앞발 뒤꿈치로 밀어 처음 자세로 돌아오고 교대.'], tip:'보폭이 좁으면 무릎에 부담이 커요.', caution:'무릎 통증 시 스플릿 스쿼트로 대체.', youtube:'런지 자세 교정 초보' },
  '불가리안 스플릿 스쿼트': { target:['대퇴사두근','둔근','햄스트링','균형'], how:['뒷발을 의자나 소파 위에 올려요 (발등이 위로).','앞발로 서서 상체를 곧게 편 채로 무릎을 구부려 내려가요.','앞 무릎이 90°가 될 때까지 내려갔다가 올라와요.','한쪽 끝나면 발 교체.'], tip:'런지보다 둔근과 햄스트링 자극이 훨씬 강해요. 처음엔 균형 잡기 힘드니 천천히!', caution:'뒷발을 너무 높이 올리면 무릎에 부담. 의자 높이 30~40cm가 적당.', youtube:'불가리안 스플릿 스쿼트 홈트' },
  '힙 브릿지 (글루트 브릿지)': { target:['둔근 (엉덩이)','햄스트링','코어 하부'], how:['등을 대고 누워 무릎을 세워요.','엉덩이를 꽉 쥐면서 골반을 천장으로 들어올려요.','어깨~무릎 일직선에서 1~2초 유지.','바닥에 닿기 전에 다시 올려요.'], tip:'올라간 상태에서 엉덩이를 최대한 쥐어짜는 느낌!', caution:'허리가 과도하게 꺾이지 않도록 복부 유지.', youtube:'글루트 브릿지 자세 홈트' },
  '싱글레그 힙 브릿지': { target:['둔근','햄스트링','코어 안정성'], how:['힙 브릿지 자세에서 한쪽 다리를 들어 쭉 뻗어요.','남은 한 발로만 바닥을 밀어 골반을 올려요.','일직선 유지 후 천천히 내려오고 발 교체.'], tip:'일반 힙 브릿지보다 둔근 자극이 2배! 처음엔 발을 완전히 펴지 않아도 돼요.', caution:'골반이 한쪽으로 기울지 않도록 코어에 힘을 유지하세요.', youtube:'싱글레그 힙 브릿지 홈트' },
  '플랭크': { target:['복직근 (앞 복부)','복횡근 (심부 코어)','어깨 안정근','둔근'], how:['팔꿈치를 어깨 바로 아래에 대고 엎드려요.','발끝 세우고 머리~발 일직선.','배꼽을 등 쪽으로 당기는 느낌으로 복부 팽팽하게.'], tip:'엉덩이가 올라가거나 처지면 효과 없어요. 거울로 확인!', caution:'허리 통증 시 니 플랭크로 시작.', youtube:'플랭크 자세 초보 교정' },
  '플랭크 숄더 탭': { target:['코어 전반','어깨 안정근','복사근'], how:['팔을 쭉 편 푸시업 준비 자세로 시작.','오른손으로 왼쪽 어깨를 터치하고 돌아와요.','반대편도 반복. 좌우 각 1회 = 1회.','골반이 흔들리지 않게 코어에 힘!'], tip:'천천히 할수록 코어 자극이 강해요. 속도보다 안정성이 핵심.', caution:'허리가 흔들리면 발 간격을 넓혀서 안정성을 높이세요.', youtube:'플랭크 숄더 탭 코어 홈트' },
  '플랭크 변형 (사이드 플랭크)': { target:['복사근 (옆 복부)','중둔근 (엉덩이 옆)','어깨 안정근'], how:['옆으로 누워 팔꿈치를 어깨 바로 아래에 대요.','발을 모으고 엉덩이를 들어 머리~발 일직선.','유지 후 반대편 교체.'], tip:'엉덩이가 처지거나 앞으로 회전하지 않게 버티는 게 핵심.', caution:'어깨가 불안정하면 팔꿈치 버전으로.', youtube:'사이드 플랭크 자세 교정' },
  '푸시업 (일반 or 무릎)': { target:['대흉근 (가슴)','전삼각근 (앞 어깨)','삼두근'], how:['손은 어깨보다 살짝 넓게, 손가락은 앞을 향해.','팔꿈치 45° 각도로 구부리며 가슴이 2~3cm 위까지 내려가요.','밀어 올리며 한 회 완성.'], tip:'팔꿈치가 너무 옆으로 벌어지지 않게 — 45°가 관절에 안전.', caution:'허리가 처지면 복부에 힘을 더 주세요.', youtube:'푸시업 자세 초보 교정' },
  '푸시업 (와이드 그립)': { target:['대흉근 내측','전삼각근','삼두근'], how:['손을 어깨 너비의 1.5배로 넓게 짚어요.','일반 푸시업과 동일하게 내려가되 팔꿈치가 더 옆으로.','내릴 때 가슴 늘어나는 느낌, 올릴 때 수축하는 느낌.'], tip:'와이드 그립은 가슴 자극이 강해요.', caution:'어깨 불편하면 그립 너비 줄이세요.', youtube:'와이드 푸시업 가슴 홈트' },
  '인클라인 푸시업 (의자)': { target:['대흉근 하부','전삼각근','삼두근'], how:['의자나 소파에 양손을 짚고 푸시업 자세를 취해요.','몸이 기울어진 상태에서 팔꿈치를 구부려 내려가요.','가슴이 의자 가장자리 가까이 올 때까지.'], tip:'각도가 낮을수록(발이 높을수록) 강도가 올라가요. 처음엔 높은 의자로!', caution:'의자가 미끄러지지 않게 고정 확인.', youtube:'인클라인 푸시업 홈트' },
  '파이크 푸시업': { target:['삼각근 (어깨 전반)','승모근','삼두근'], how:['엉덩이를 높이 들어 역V자 모양.','머리가 손 사이 바닥을 향하도록 팔꿈치를 구부려 내려가요.','팔꿈치 펴서 올라오면 한 회.'], tip:'엉덩이가 낮아지면 어깨가 아닌 가슴 운동이 돼요.', caution:'어깨 통증 시 즉시 중단.', youtube:'파이크 푸시업 어깨 홈트' },
  '다이아몬드 푸시업': { target:['삼두근 (팔 뒤쪽)','대흉근 내측'], how:['손을 가슴 아래에서 엄지와 검지로 다이아몬드 모양을 만들어요.','팔꿈치를 몸통 가까이 붙인 채로 구부려 내려가요.','삼두로 밀어 올려요.'], tip:'삼두 자극이 일반 푸시업보다 훨씬 강해요. 처음엔 무릎 대고 해도 OK.', caution:'손목 통증 시 주먹 쥐고 하거나 중단.', youtube:'다이아몬드 푸시업 삼두 홈트' },
  '트라이셉스 딥 (의자)': { target:['삼두근 (팔 뒤쪽)','전삼각근'], how:['의자 끝에 양손을 짚고 엉덩이를 앞으로 내려요.','팔꿈치 90°까지 구부리며 몸을 내렸다가 삼두로 밀어 올려요.'], tip:'내려갈 때 팔꿈치가 뒤쪽으로 향하도록 유지.', caution:'어깨 통증 시 깊이 줄이거나 중단.', youtube:'트라이셉스 딥 의자 홈트' },
  '크런치 + 레그레이즈': { target:['복직근 (윗 복부)','장요근 (하복부)'], how:['[크런치] 무릎 세우고 누워서 상체를 말아 올려요.','[레그레이즈] 다리 펴고 90°까지 올렸다가 천천히 내려요.','크런치 15회 → 레그레이즈 15회를 한 세트로.'], tip:'크런치는 복부로 말아 올리는 느낌. 목을 당기면 안 돼요!', caution:'허리 통증 시 레그레이즈는 무릎 구부린 채로.', youtube:'크런치 레그레이즈 복근 홈트' },
  '볼 레이즈 크런치': { target:['복직근 (상복부)','코어 전반'], how:['등을 대고 무릎 세워 누워요.','메디신볼을 가슴 위에 두 손으로 들어요.','볼을 천장 쪽으로 밀며 상체를 말아 올려요.','천천히 내려오며 복근 긴장 유지.'], tip:'볼 무게가 추가 저항이 되어 일반 크런치보다 자극이 강해요.', caution:'목을 당기지 말고 복부로 말아 올리는 느낌에 집중.', youtube:'볼 크런치 복근 홈트' },
  '러시안 트위스트 (볼 홀드)': { target:['복사근 (옆 복부)','복직근','코어 회전력'], how:['바닥에 앉아 무릎 살짝 구부리고 발을 들어요.','볼을 양손으로 들고 상체를 약간 뒤로 기울여요.','볼을 좌우로 번갈아 바닥에 터치하며 비틀어요.'], tip:'팔만 이동하지 말고 상체 전체가 회전해야 복사근 자극!', caution:'허리 디스크가 있으면 회전 범위 줄이세요.', youtube:'러시안 트위스트 복사근' },
  '고블릿 스쿼트 (볼 홀드)': { target:['대퇴사두근','둔근','코어','상체 안정근'], how:['볼을 두 손으로 가슴 앞에 세로로 들어요.','발을 어깨보다 살짝 넓게, 발끝 약간 바깥.','허벅지가 바닥과 평행해질 때까지 내려갔다가 올라와요.'], tip:'일반 스쿼트보다 더 깊이 앉기 쉽고 자연스럽게 정렬돼요.', caution:'무게가 무거우면 상체가 앞으로 쏠려요. 처음엔 가볍게.', youtube:'고블릿 스쿼트 초보' },
  '오버헤드 스쿼트 (볼 홀드)': { target:['전신 (하체+코어+어깨 안정근)','균형감각'], how:['볼을 양손으로 머리 위로 들어 올려요.','팔을 쭉 뻗은 채 유지하면서 스쿼트로 내려가요.','허벅지 평행까지 내려갔다가 올라와요.'], tip:'코어와 어깨 안정근이 동시에 자극돼요. 가벼운 볼로 시작!', caution:'어깨 불편하면 고블릿 스쿼트로 대체.', youtube:'오버헤드 스쿼트 홈트' },
  '사이드 런지 + 볼 홀드': { target:['내전근 (허벅지 안쪽)','둔근','대퇴사두근'], how:['볼을 가슴 앞에 들고 서요.','오른발을 옆으로 크게 내딛으며 오른 무릎을 구부려 앉아요.','왼발은 쭉 뻗은 채 유지. 밀어 돌아오고 교대.'], tip:'볼을 앞에 들면 상체 균형 잡기가 쉬워요.', caution:'무릎 통증 시 내딛는 폭을 줄이세요.', youtube:'사이드 런지 홈트' },
  '루마니안 데드리프트 (볼 홀드)': { target:['햄스트링 (뒤 허벅지)','둔근','척추기립근 (허리 등)'], how:['볼을 양손으로 허벅지 앞에 들고 서요.','등을 곧게 편 채로 엉덩이를 뒤로 밀며 상체를 앞으로 숙여요.','볼이 정강이 중간쯤 내려오면 둔근을 조이며 올라와요.','무릎은 살짝만 구부려요.'], tip:'허리가 아닌 햄스트링이 당기는 느낌이 나야 정확한 자세예요.', caution:'허리를 둥글게 구부리면 부상 위험. 등을 항상 곧게!', youtube:'루마니안 데드리프트 홈트' },
  '스텝박스 스텝업 인터벌': { target:['대퇴사두근','둔근','심폐 지구력'], how:['박스 앞에 서서 오른발 → 왼발 순서로 올라가고 내려와요.','강한 구간에서 최대한 빠르게 반복.','내려올 때 착지 소리 없게 부드럽게.'], tip:'팔을 앞뒤로 흔들면 자연스럽게 속도가 올라가요.', caution:'피로 상태에서 박스 가장자리 밟지 않도록 주의!', youtube:'스텝박스 인터벌 홈트' },
  '스텝박스 스텝업 (빠르게)': { target:['대퇴사두근','둔근','심폐 지구력'], how:['박스 앞에서 최대한 빠르게 올라갔다 내려오기 반복.','서킷 중이므로 정해진 시간 안에 최대한 많이.'], tip:'팔 흔들기로 속도 높이기!', caution:'지친 상태에서 발 위치 꼭 확인.', youtube:'스텝박스 스텝업 빠르게' },
  '스텝박스 스텝업 (무게 추가)': { target:['대퇴사두근','둔근','햄스트링','균형'], how:['볼을 가슴에 안고 박스 앞에 서요.','오른발 → 왼발 순서로 올라가고 내려와요.','한쪽 10~14회 끝나면 발 교체.'], tip:'무게 들면 균형이 어려워요. 천천히 안전하게!', caution:'박스가 흔들리지 않는지 확인. 착지 소리 없게.', youtube:'스텝박스 가중 스텝업' },
  '스텝퍼 유산소': { target:['심폐 지구력','하체 근지구력','칼로리 소모'], how:['루틴 노트에 표시된 심박 구간을 목표로 밟아요.','손잡이는 최대한 가볍게 또는 잡지 않기.'], tip:'손잡이를 놓으면 코어 자극과 칼로리 소모가 올라가요.', caution:'어지럼증 시 즉시 속도 줄이기.', youtube:'스텝퍼 유산소 운동' },
  '스텝퍼 중강도 유산소': { target:['심폐 지구력','하체 근지구력'], how:['분당 80~90 스텝, 심박 130~148 구간 유지.','손잡이 가볍게만 터치.'], tip:'숨이 차지만 짧은 문장은 말할 수 있는 강도가 적정!', caution:'심박 165 이상이면 속도 줄이세요.', youtube:'스텝퍼 중강도 유산소' },
  '스텝퍼 고중강도 유산소': { target:['심폐 지구력','칼로리 소모'], how:['분당 90~100 스텝, 심박 140~150 구간 유지.','손잡이 잡지 않으면 칼로리 소모 15~20% 증가!'], tip:'숨이 많이 차지만 완전히 지치지는 않는 강도.', caution:'심박 165 이상 올라가면 속도 줄이세요.', youtube:'스텝퍼 고강도 유산소' },
  '스텝퍼 HIIT': { target:['심폐 극대화','지방 연소 가속'], how:['고강도 구간: 분당 100~110 스텝, 최대한 빠르게.','저강도 구간: 분당 60 스텝, 심박 회복.','번갈아 반복. 마지막 2회는 최대 강도로!'], tip:'HIIT는 고강도 구간을 진짜 힘들게 해야 효과 있어요.', caution:'처음엔 6회부터 시작하고 매주 2회씩 늘려가세요.', youtube:'스텝퍼 HIIT 인터벌 운동' },
  '스텝퍼 저강도 유산소': { target:['회복 촉진','심폐 유지','하체 혈류'], how:['분당 60~70 스텝, 심박 110~125 구간.','30~35분 동안 꾸준히.'], tip:'팟캐스트나 음악 들으며 가볍게!', caution:'리커버리 날은 강도 높이는 충동을 참으세요.', youtube:'액티브 리커버리 운동' },
  '마운틴 클라이머': { target:['코어 전반','어깨 안정근','심폐 지구력'], how:['푸시업 준비 자세로 손을 바닥에 짚어요.','오른쪽 무릎을 가슴으로 당겼다가, 왼쪽 무릎을 당겨요.','달리듯 좌우를 빠르게 교대.'], tip:'엉덩이가 올라가지 않게 플랭크 자세를 유지!', caution:'손목 약하면 주먹 쥐고 하거나 인클라인으로.', youtube:'마운틴 클라이머 자세 홈트' },
  '버피 (변형: 점프 없이)': { target:['전신 (하체+상체+심폐)'], how:['서서 손을 바닥에 짚고 발을 뒤로 뻗어 플랭크.','발을 다시 손 옆으로 당겨요.','점프 없이 일어나면서 손을 머리 위로.'], tip:'빠르게 하면 점프 없어도 심박이 충분히 올라요.', caution:'손목 약하면 주먹 쥐고 하거나 푸시업 손잡이 활용.', youtube:'버피 변형 점프없이 초보' },
  '버피 (표준)': { target:['전신 (하체+상체+심폐)','칼로리 소모 최대'], how:['손 짚고 플랭크 → 푸시업(선택) → 발 당겨 → 점프하며 손 머리 위로.','착지 즉시 다음 회 이어가요.'], tip:'자세를 지키는 게 속도보다 중요해요.', caution:'심폐 부담 크므로 어지럼증 시 즉시 중단.', youtube:'버피 표준 자세 전신' },
  '점핑잭 (저강도 버전)': { target:['심폐 지구력','전신 혈류'], how:['발 모으고 팔 몸 옆에서 시작.','점프하며 발 벌리고 팔을 머리 위로.','다시 점프해서 처음 자세로.'], tip:'서킷 사이 회복 동작으로도 좋아요.', caution:'층간 소음 걱정되면 점프 없는 사이드 스텝으로.', youtube:'점핑잭 저강도 홈트' },
  '슈퍼세트 A — 스쿼트 + 힙브릿지': { target:['대퇴사두근','둔근','햄스트링','코어'], how:['스쿼트 지정 횟수를 바로 실시.','쉬지 않고 바닥에 누워 힙 브릿지 지정 횟수 이어서.','두 동작 끝나면 지정 휴식 후 다음 세트.'], tip:'슈퍼세트는 쉬지 않고 이어가는 게 핵심! 엉덩이와 허벅지가 모두 타는 느낌.', caution:'처음 슈퍼세트라면 세트 수를 2~3개부터 시작.', youtube:'스쿼트 힙브릿지 슈퍼세트' },
  '슈퍼세트 B — 푸시업 + 파이크 푸시업': { target:['대흉근 (가슴)','삼각근 (어깨)','삼두근'], how:['푸시업 지정 횟수 바로 실시.','쉬지 않고 엉덩이 들어 파이크 자세로 이어서 실시.','두 동작 끝나면 지정 휴식 후 다음 세트.'], tip:'가슴 → 어깨로 피로가 전환되어 두 부위 모두 자극돼요.', caution:'어깨 통증 시 즉시 중단.', youtube:'푸시업 파이크 슈퍼세트' },
  '서킷 3라운드 (60초 휴식)': { target:['전신','심폐 지구력'], how:['표시된 동작들을 연속으로 실시.','한 라운드 끝나면 60초 휴식 후 반복.'], tip:'동작 사이에 멈추지 않는 것이 핵심!', caution:'너무 힘들면 각 동작 시간을 줄여도 돼요.', youtube:'홈트 서킷 3라운드' },
  '서킷 4라운드 (45초 휴식)': { target:['전신','심폐 지구력'], how:['표시된 동작들을 연속으로 실시.','라운드 사이 45초 휴식.'], tip:'라운드 늘어날수록 힘들지만 마지막 라운드를 제일 세게!', caution:'어지럼증 시 즉시 중단.', youtube:'홈트 서킷 4라운드' },
  '서킷 4라운드 (30초 휴식)': { target:['전신','심폐 지구력'], how:['표시된 동작들을 연속으로 실시.','라운드 사이 30초 휴식.'], tip:'30초는 매우 짧아요. 빠른 전환 연습!', caution:'어지럼증 시 즉시 중단.', youtube:'홈트 서킷 4라운드' },
  '서킷 5라운드 (30초 휴식)': { target:['전신','심폐 피크'], how:['표시된 동작들을 연속으로 실시.','라운드 사이 30초만 휴식.'], tip:'5라운드는 상당히 힘들어요. 포기하지 마세요!', caution:'어지럼증 시 즉시 중단.', youtube:'홈트 서킷 5라운드' },
  '서킷 5라운드 (20초 휴식)': { target:['전신','심폐 피크'], how:['표시된 동작들을 연속으로 실시.','라운드 사이 20초만 휴식.'], tip:'20초는 물 한 모금 마실 시간. 빠르게 다음 라운드!', caution:'어지럼증 시 즉시 중단.', youtube:'홈트 서킷 고강도' },
  '서킷 6라운드 (15초 휴식)': { target:['전신','심폐 극한'], how:['표시된 동작들을 연속으로 실시.','라운드 사이 15초만 휴식.'], tip:'심화기 최고 강도 서킷. 완주 자체가 목표!', caution:'어지럼증 시 즉시 중단.', youtube:'홈트 서킷 심화' },
  '전신 스트레칭': { target:['근막 이완','유연성 향상','피로 회복'], how:['목→어깨→가슴→등→허리→엉덩이→허벅지→종아리 순서.','각 부위 15~30초 유지.'], tip:'유튜브에서 15분 전신 스트레칭 루틴 틀어놓고 따라 하면 쉬워요.', caution:'반동으로 억지로 늘리면 부상 위험.', youtube:'전신 스트레칭 15분 루틴' },
  '쿨다운 스트레칭': { target:['근육 이완','젖산 제거','유연성 유지'], how:['운동 직후 제자리 걷기 1~2분으로 심박 낮추기.','오늘 사용한 부위 위주로 15~20초씩 스트레칭.'], tip:'운동 끝나도 바로 앉지 말고 5분 이상 움직이며 식히세요.', caution:'통증 있는 부위는 과하게 스트레칭 금지.', youtube:'운동 후 쿨다운 스트레칭' },
};

const GUIDES = [
  { icon:'🏃', title:'스텝퍼 활용법', content:`<ul><li><strong>속도</strong>: 워밍업 65~70, 중강도 80~90, 고강도 100, HIIT 110 이상 (분당 스텝)</li><li><strong>자세</strong>: 등 곧게, 발 전체로 밟기, 손잡이는 가볍게만</li><li><strong>효과 극대화</strong>: 손잡이 잡지 않으면 코어↑ 칼로리↑</li><li><strong>심박 목표</strong>: 중강도 130~145, 고강도 148~158, HIIT 피크 160+ bpm</li><li>칼로리 소모 (112kg 기준): 중강도 20분 ≈ 250~300 kcal</li></ul>` },
  { icon:'📦', title:'스텝박스 활용법', content:`<ul><li><strong>기본 스텝업</strong>: 오른발→왼발 올라가고 내려오기</li><li><strong>인터벌</strong>: 강한 구간/약한 구간 비율은 주차별 루틴 참고</li><li><strong>박스 높이</strong>: 중급 25~35cm</li><li><strong>유튜브 추천</strong>: "스텝박스 유산소 30분", "step box HIIT" 검색</li><li>무릎 아프면 즉시 중단!</li></ul>` },
  { icon:'⚽', title:'볼 운동 활용법 (소음 없는 동작)', content:`<ul><li><strong>고블릿 스쿼트</strong>: 볼 가슴 앞에 들고 스쿼트</li><li><strong>오버헤드 스쿼트</strong>: 볼 머리 위로 들고 스쿼트 — 전신+코어</li><li><strong>루마니안 데드리프트</strong>: 볼 들고 엉덩이 뒤로 밀기 — 햄스트링</li><li><strong>볼 레이즈 크런치</strong>: 누워서 볼 들고 크런치 — 복근</li><li><strong>사이드 런지 + 볼 홀드</strong>: 볼 들고 옆 런지 — 내전근·둔근</li><li><strong>러시안 트위스트</strong>: 앉아서 볼 좌우 터치 — 복사근</li><li>권장 무게: 4kg → 6kg → 8kg → 10kg</li></ul>` },
  { icon:'💡', title:'알배김(DOMS) 관리법', content:`<ul><li><strong>왜 심한가?</strong>: 근육량 많을수록 회복에 단백질·산소가 더 필요해요</li><li><strong>예방</strong>: 쿨다운 7분 이상 + 운동 후 30분 내 단백질 섭취</li><li><strong>완화</strong>: 폼롤러로 뭉친 부위 30초씩 롤링, 따뜻한 샤워</li><li><strong>수분</strong>: 운동 전중후 물 2.5L 이상</li><li>알배김 심한 날도 가벼운 스트레칭은 오히려 회복에 도움!</li></ul>` },
  { icon:'🍽️', title:'식단 가이드', content:`<ul><li><strong>하루 칼로리</strong>: 1,800~2,000 kcal</li><li><strong>단백질</strong>: 하루 150~200g (닭가슴살, 계란, 두부, 그릭요거트)</li><li><strong>탄수화물</strong>: 현미, 고구마, 오트밀로 대체</li><li><strong>운동 전 30분</strong>: 바나나 1개 or 고구마 소량</li><li><strong>운동 후 30분</strong>: 단백질 위주 식사 (알배김 예방 핵심!)</li><li>물 하루 2.5L 이상 필수!</li></ul>` },
  { icon:'📅', title:'주차별 강도 로드맵', content:`<ul><li><strong>1~2주 (심폐 적응기)</strong>: 하체A·상체A·서킷A — 기본 자세 완성</li><li><strong>3~4주 (지구력 강화기)</strong>: 하체B·상체B·서킷B — 종목 교체 + 휴식 단축</li><li><strong>5~6주 (복합 강화기)</strong>: 하체C·상체C·서킷C — 슈퍼세트 도입</li><li><strong>7~8주 (HIIT 집중기)</strong>: 하체D·상체D·서킷D — 점프 동작 + HIIT</li><li><strong>9~10주 (피크 강도기)</strong>: 하체E·상체E·서킷E — 전신 복합 피크</li><li><strong>11주~ (유지·심화기)</strong>: 하체F·상체F·서킷F — 무게 증가 심화</li></ul>` },
];

// ===== STATE =====
let state = { completedDays:{}, checkedExercises:{}, weightLog:[], sheetsUrl:'', currentWeekOffset:0 };
function saveState() { localStorage.setItem('fitnessPlannerState_v4', JSON.stringify(state)); }
function loadState() { const raw=localStorage.getItem('fitnessPlannerState_v4'); if(raw){try{Object.assign(state,JSON.parse(raw));}catch(e){}} }
...
function renderExerciseList(exercises, todayKey, isInteractive){
  const tagMap={warm:'워밍업',cool:'쿨다운',cardio:'유산소',strength:'근력',core:'코어'};
  const checked=isInteractive?(state.checkedExercises[todayKey]||{}):{};
  return exercises.map((ex,i)=>{
    const isDone=isInteractive&&!!checked[i];
...
function renderToday(){
  ...
  list.innerHTML=renderExerciseList(plan.exercises,todayKey,true);
